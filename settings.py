...
class Settings(QWidget):
    ...
    def createVerticalLayout(self):
        self.horizontalGroupBox = QGroupBox("Leak Test Settings")
        layout = QGridLayout()
        ...
        layout.addWidget(lastIDLine,0,1,1,2)

        layout.addWidget(dateFormat, 1, 0)
        layout.addWidget(dateFormatLine,1,1,1,2)

        layout.addWidget(reportNameFormat, 2, 0)
        layout.addWidget(reportNameFormatLine, 2, 1, 1, 2)

        layout.addWidget(reportPath, 3, 0)
        layout.addWidget(reportPathLine, 3, 1, 1, 2)

        layout.addWidget(soundAlert, 4, 0)
        layout.addWidget(soundAlertLine, 4, 1, 1, 2)

        layout.addWidget(testTime, 5, 0)
        layout.addWidget(testTimeLine, 5, 1, 1, 2)

        layout.addWidget(pressureUnits, 6, 0)
        layout.addWidget(pressureUnitsLine, 6, 1, 1, 2)

        layout.addWidget(leakRateUnits, 7, 0)
        layout.addWidget(leakRateUnitsLine, 7, 1, 1, 2)

        layout.addWidget(testButton, 8, 0)
        layout.setColumnStretch(0,2)

        layout.addWidget(saveButton, 8, 1)
        layout.addWidget(closeButton, 8, 2)

        self.horizontalGroupBox.setLayout(layout)

        closeButton.clicked.connect(self.close)
        #closeButton.clicked.connect(self.parentWidget().close())
